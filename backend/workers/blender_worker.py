...
def generate_walls(bw, bd, num_floors, floor_h, base_z, M, sc_cfg, style_name):
    ...
    for fi in range(num_floors):
        ...
        if style_name == "industrial":
            ...
            band_mat  = M.get("steel") or M["accent"]
            trim_mat  = M.get("corten") or M["accent"]
        elif style_name == "japanese":
            wall_mat  = M["wall_wood"] if fi == 0 else M["wall"]
            band_mat  = M["wood_dark"]
            trim_mat  = M["wood_dark"]
        elif style_name == "villa":
            wall_mat  = M["plaster"]
            band_mat  = M.get("terracotta") or M["accent"]
            trim_mat  = M.get("wood_dark") or M["facade"]
        elif style_name == "scandinavian":
            wall_mat  = M["facade"]
            band_mat  = M["accent"]
            trim_mat  = M["wood_light"]
        elif style_name == "colonial":
            wall_mat  = M["facade"]
            band_mat  = M["accent"]
            trim_mat  = M["white_paint"]
        elif style_name == "classical":
            wall_mat  = M["facade"]
            band_mat  = M.get("marble") or M["white_paint"]
            trim_mat  = M["marble"]
        elif style_name == "asian":
            wall_mat  = M["facade"]
            band_mat  = M["accent"]
            trim_mat  = M.get("wood_dark") or M["accent"]
        else:
            wall_mat  = M["facade"]
            band_mat  = M.get("accent") or M["facade"]
            trim_mat  = M.get("wood_dark") or M["facade"]
        ...
        # ── Exterior accent band at each floor level ──────────────────────────
        band_z = fz + floor_h - 0.04
        bh = 0.08
        add_box(f"Band_N_{fi}", (0, bd/2, band_z), (bw + wt, bh, bh), band_mat)
        add_box(f"Band_S_{fi}", (0, -bd/2, band_z), (bw + wt, bh, bh), band_mat)
        add_box(f"Band_E_{fi}", (bw/2, 0, band_z), (bh, bd + wt, bh), band_mat)
        add_box(f"Band_W_{fi}", (-bw/2, 0, band_z), (bh, bd + wt, bh), band_mat)

        # ── Corner pilasters / trim ──────────────────────────────────────────
        pt = 0.12
        for xi, yi in [(1,1),(1,-1),(-1,1),(-1,-1)]:
            cx = xi * (bw/2 - pt/2)
            cy = yi * (bd/2 - pt/2)
            add_box(f"Pil_{fi}_{xi}_{yi}", (cx, cy, fz + floor_h/2), (pt, pt, floor_h), trim_mat)

    # ── Base course at ground level ──────────────────────────────────────────
    base_mat = band_mat
    base_z_pos = base_z - 0.02
    add_box("Base_Course", (0, 0, base_z_pos + 0.08), (bw + 0.2, bd + 0.2, 0.16), base_mat)
...
def _build_interior_walls(rooms, iw, id_, fi, fz, floor_h, M):
    """Create walls between adjacent rooms with door gaps."""
    iw_mat = M.get("interior_wall") or M["wall"]
    wt = 0.16
    dw = 0.9
    dh = 2.2
...
def _place_room_furniture(room_type, cx, cy, w, d, fi, fz, M, style_name):
    """Place furniture appropriate to the room type."""
    ...
    if room_type == "living":
        sx = cx - w * 0.1
        sy = cy - d * 0.15
        add_box(f"Sofa_{fi}", (sx, sy, fz + 0.22), (2.4, 0.9, 0.44), fab)
        add_box(f"SofaBack_{fi}", (sx, sy - 0.4, fz + 0.5), (2.4, 0.15, 0.6), fab)
        add_box(f"Coffee_{fi}", (sx - 0.4, sy + 0.5, fz + 0.22), (1.0, 0.6, 0.04), wood)
        add_box(f"TVUnit_{fi}", (cx + w * 0.35, cy, fz + 0.3), (1.6, 0.4, 0.5), wood)
        add_box(f"TVScreen_{fi}", (cx + w * 0.35 + 0.2, cy, fz + 0.88), (1.2, 0.06, 0.8),
                mat("tv", (0.05, 0.05, 0.05), rough=0.1))
        lm = M.get("lantern")
        if lm:
            lx = cx - w * 0.25
            if style_name == "japanese":
                add_cylinder(f"Lant_{fi}", (lx, cy + d * 0.3, fz + 0.55), 0.15, 0.55, verts=8, mat=lm)
                add_box(f"LantBase_{fi}", (lx, cy + d * 0.3, fz + 0.02), (0.25, 0.25, 0.04), dark or wood)
            elif style_name == "industrial":
                add_cylinder(f"Lant_{fi}", (cx, cy - d * 0.1, fz + 1.8), 0.08, 0.25, verts=8, mat=lm)
                add_box(f"LantCord_{fi}", (cx, cy - d * 0.1, fz + 2.5), (0.02, 0.02, 1.4), M.get("steel") or dark)
            else:
                add_cylinder(f"Lant_{fi}", (lx, cy + d * 0.3, fz + 0.9), 0.12, 0.45, verts=8, mat=lm)
                add_box(f"LantPole_{fi}", (lx, cy + d * 0.3, fz + 0.38), (0.04, 0.04, 1.2), M.get("steel") or dark)

    elif room_type == "dining":
        add_box(f"DTable_{fi}", (cx, cy, fz + 0.38), (1.6, 0.9, 0.04), wood)
        for di, (dx, dy) in enumerate([(-0.6, -0.25), (-0.6, 0.25), (0.6, -0.25), (0.6, 0.25)]):
            add_box(f"DChair_{fi}_{di}", (cx + dx, cy + dy, fz + 0.25), (0.45, 0.45, 0.44), fab)

    elif room_type == "kitchen":
        add_box(f"Counter_{fi}", (cx + w * 0.3, cy, fz + 0.5), (1.8, 0.6, 1.0), wood)
        add_box(f"CounterTop_{fi}", (cx + w * 0.3, cy, fz + 1.0), (1.8, 0.6, 0.04), M.get("marble") or wood)
        add_box(f"Fridge_{fi}", (cx - w * 0.3, cy, fz + 0.8), (0.8, 0.8, 1.6), M.get("steel") or dark)

    elif room_type == "entry":
        add_box(f"EntryTable_{fi}", (cx, cy, fz + 0.45), (0.8, 0.35, 0.04), wood)

    elif room_type in ("bedroom", "master", "bed_top"):
        mat_m = mat(f"mat_{fi}", (0.95, 0.93, 0.90), rough=0.9)
        mat_p = mat(f"pil_{fi}", (0.98, 0.96, 0.94), rough=0.95)
        bx = cx - w * 0.15
        by = cy + d * 0.1
        add_box(f"Bed_{fi}", (bx, by, fz + 0.18), (1.6, 2.0, 0.36), wood)
        add_box(f"Head_{fi}", (bx, by + 1.0, fz + 0.55), (1.6, 0.12, 0.74), dark)
        add_box(f"Mat_{fi}", (bx, by, fz + 0.38), (1.55, 1.9, 0.28), mat_m)
        add_box(f"PillL_{fi}", (bx - 0.38, by + 0.6, fz + 0.54), (0.6, 0.4, 0.16), mat_p)
        add_box(f"PillR_{fi}", (bx + 0.38, by + 0.6, fz + 0.54), (0.6, 0.4, 0.16), mat_p)
        add_box(f"Ward_{fi}", (cx + w * 0.3, cy - d * 0.35, fz + 1.0), (1.2, 0.5, 2.0), dark)
        add_box(f"Desk_{fi}", (cx + w * 0.25, cy + d * 0.25, fz + 0.38), (1.0, 0.5, 0.04), wood)
        add_box(f"Mon_{fi}", (cx + w * 0.25, cy + d * 0.25 + 0.2, fz + 0.72), (0.55, 0.05, 0.38),
                mat(f"mon_{fi}", (0.05, 0.05, 0.05), rough=0.1))

    elif room_type in ("bathroom", "bath_top"):
        add_box(f"Vanity_{fi}", (cx - w * 0.2, cy + d * 0.25, fz + 0.45), (1.2, 0.5, 0.9), wood)
        add_box(f"Toilet_{fi}", (cx + w * 0.2, cy - d * 0.15, fz + 0.2), (0.45, 0.4, 0.4), M.get("white_paint") or fab)
        add_box(f"Tub_{fi}", (cx - w * 0.15, cy - d * 0.2, fz + 0.2), (1.4, 0.7, 0.4), M.get("marble") or wood)

    elif room_type == "study":
        for di in range(2):
            dx = -w * 0.2 + di * w * 0.3
            add_box(f"Desk_{fi}_{di}", (cx + dx, cy, fz + 0.38), (1.2, 0.6, 0.04), wood)
            add_box(f"Mon_{fi}_{di}", (cx + dx, cy - 0.1, fz + 0.72), (0.5, 0.05, 0.38),
                    mat(f"mon_{fi}_{di}", (0.05, 0.05, 0.05), rough=0.1))

    elif room_type == "powder":
        add_box(f"PVanity_{fi}", (cx - w * 0.15, cy + d * 0.2, fz + 0.45), (0.8, 0.4, 0.9), wood)


def _floor_mat_for_room(room_type, M, style_name):
    """Return floor material appropriate to room type."""
    if room_type in ("kitchen", "powder", "bathroom", "bath_top"):
        return M.get("marble") or M.get("interior_floor") or M["facade"]
    elif room_type == "entry":
        return M.get("stone_zen") or M.get("interior_floor") or M["facade"]
    elif room_type == "living":
        if style_name == "japanese":
            return M.get("tatami") or M.get("interior_floor") or M["wood_light"]
        return M.get("interior_floor") or M.get("wood_light") or M["facade"]
    elif room_type in ("bedroom", "master", "bed_top"):
        if style_name == "japanese":
            return M.get("tatami") or M.get("interior_floor") or M["wood_light"]
        return M.get("interior_floor") or M.get("wood_light") or M["facade"]
...
def generate_interior(bw, bd, num_floors, floor_h, base_z, M, style_name):
    """Generate practical room-based floor plans with walls, furniture, and per-room materials."""
    wall_t = 0.28
    iw = bw - wall_t * 2
    id_ = bd - wall_t * 2
    ...
    for fi in range(num_floors):
        ...
        # Ceiling
        add_box(f"Ceiling_{fi}", (0, 0, fz + floor_h - 0.05), (iw, id_, 0.06), ceil_mat)

        # Ceiling features per style
        _add_ceiling_features(iw, id_, fi, fz, floor_h, M, style_name)

        # Furniture per room
        for room_type, cx, cy, w, d in rooms:
            _place_room_furniture(room_type, cx, cy, w, d, fi, fz + slab_t, M, style_name)
...
def _add_ceiling_features(inner_w, inner_d, fi, fz, floor_h, M, style_name):
    ...
    if style_name == "japanese":
        ...
        for yi, y in enumerate((-inner_d*0.25, inner_d*0.25)):
            add_box(f"CGrid_H_{fi}_{yi}", (0, y, cz), (inner_w, bt, bh*0.6), grid)
        for xi, x in enumerate((-inner_w*0.25, inner_w*0.25)):
            add_box(f"CGrid_V_{fi}_{xi}", (x, 0, cz), (bt, inner_d, bh*0.6), grid)

    elif style_name == "scandinavian":
        beam = M.get("wood_light") or M["facade"]
        for yi, y in enumerate((-inner_d*0.12, inner_d*0.12)):
            add_box(f"CBeam_{fi}_{yi}", (0, y, cz), (inner_w, bt, bh*0.5), beam)
