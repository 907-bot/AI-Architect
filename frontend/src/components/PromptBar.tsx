  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        const httpData = payload.scene_data || data || {};
        ...
        if (eventType === "agent.geometry" && data) {
          ...
          updateScene(
            { meshes: data.meshes || geo.meshes || [{ id: "building_base", type: "box", position: [0, 1.5, 0], scale: [8, 3, 10], material_id: "plaster_white" }], rooms },
            { drone_path: [] },
            { materials: data.materials || [] }
          );
        }

        if (agent === "evaluation" && msg.includes("complete")) {
          if (data || httpData) {
            updateScene(
              { meshes: geo.meshes || [{ id: "base", type: "box", position: [0, 1.5, 0], scale: [8, 3, 10], material_id: "plaster_white" }] },
              { drone_path: data?.bear?.drone_path || [] },
              { materials: httpData.materials || data?.materials || [] }
            );
          }
